...
export default function App() {
  ...
  const askQuestion = async (overrideVaultId = null, overrideQuestion = null) => {
    ...
    try {
      ...
      // TEMP DEBUG — remove after diagnosing General provisions issue
      const adMSection = indexSummary.split("DOCUMENT:").find(s => /(?:document|part)[\s_-]*m\b/i.test(s));
      if (adMSection) {
        const adMLines = adMSection.split("\n").map(l => l.trim()).filter(Boolean);
        console.log("[AD-M filename]", adMLines[0]);
        console.log("[AD-M headings near M4(3)]", adMLines.filter(l => /m4|general|sanitary|wheelchair|3\.(3[5-9]|4[0-5])/i.test(l)));
        window.__debugADMIndex = adMSection; // type: copy(window.__debugADMIndex) in console to copy full index
      } else {
        console.log("[AD-M not found] document names in vault:", indexSummary.match(/DOCUMENT:[^\n]+/g));
      }
      ...
      // TEMP DEBUG — log Pass 1 scored sections for AD M
      const adMScored = (scoring.selectedDocs || []).find(d => /(?:document|part)[\s_-]*m\b/i.test(d.docName));
      console.log("[Pass1 scored AD-M sections]", JSON.stringify(adMScored?.sections?.map(s => ({ h: s.heading, p: s.pageHint, prob: s.probability })) || [], null, 2));
