...
import { ARC_NAVY, ARC_TERRACOTTA, ARC_STONE, AD_GREEN, AD_GREEN_LIGHT } from "../constants";
...
function NewProjectForm({ onSave, onCancel }) {
  const [form, setForm] = useState({ name: "", job_number: "", client: "", location: "", stage: "", description: "" });
    ...
    <div style={{ background: "#fff", border: `1px solid #e8e0d5`, borderTop: `3px solid ${ARC_TERRACOTTA}`, padding: "28px 32px", marginBottom: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 24px" }}>
        <div>
          ...
          <input value={form.name} onChange={e => set("name", e.target.value)} placeholder="e.g. 14 Station Road" style={inputStyle} autoFocus />
        </div>
        <div>
          <label style={labelStyle}>Job Number</label>
          <input value={form.job_number} onChange={e => set("job_number", e.target.value)} placeholder="e.g. 2024-042" style={inputStyle} />
        </div>
        <div>
        ...
        <div>
          ...
          <input value={form.location} onChange={e => set("location", e.target.value)} placeholder="Town / county" style={inputStyle} />
        </div>
        <div>
        ...
        <div>
          ...
          <input value={form.description} onChange={e => set("description", e.target.value)} placeholder="Brief description" style={inputStyle} />
        </div>
      </div>
      <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
        ...
        <button className="btn" onClick={onCancel}
          style={{ background: "none", color: "#9a9088", padding: "9px 16px", fontSize: 11, border: "1px solid #ddd8d0" }}>
          Cancel
        </button>
      </div>
    </div>
...
function ProjectCard({ project, onClick }) {
  ...
  return (
    <div className="btn" onClick={onClick}
      ...
      onMouseLeave={e => { e.currentTarget.style.borderLeftColor = color; e.currentTarget.style.borderTopColor = "#e8e0d5"; e.currentTarget.style.borderRightColor = "#e8e0d5"; e.currentTarget.style.borderBottomColor = "#e8e0d5"; }}>
...
function ProjectDetail({ projectId, onBack, isAdmin }) {
  ...
  async function saveField(field, value) {
    setSavingKey(field, true);
    try {
      const { project } = await api(`/api/projects/${projectId}`, { method: "PATCH", body: { [field]: value } });
      setData(d => ({ ...d, project }));
    } catch (e) { console.error(e); }
    setSavingKey(field, false);
  }

  async function saveEditForm() {
    setSavingKey("editForm", true);
    try {
      const { project } = await api(`/api/projects/${projectId}`, { method: "PATCH", body: editForm });
      setData(d => ({ ...d, project }));
  ...
  async function updateConsultant(cid, field, value) {
    ...
    try {
      await api(`/api/projects/${projectId}/consultants/${cid}`, { method: "PATCH", body: updated });
      setData(d => ({ ...d, consultants: d.consultants.map(c => c.id === cid ? updated : c) }));
    } catch (e) { console.error(e); }
  }

  async function deleteConsultant(cid) {
    try {
      await api(`/api/projects/${projectId}/consultants/${cid}`, { method: "DELETE" });
      setData(d => ({ ...d, consultants: d.consultants.filter(c => c.id !== cid) }));
    } catch (e) { console.error(e); }
  }
  ...
  async function updateUvalue(uid, field, value) {
    ...
    try {
      const { uvalue } = await api(`/api/projects/${projectId}/uvalues/${uid}`, { method: "PATCH", body: { [field]: field === "notes" ? value : parsed } });
      setData(d => ({ ...d, uvalues: d.uvalues.map(u => u.id === uid ? uvalue : u) }));
    } catch (e) { console.error(e); }
  }
  ...
  async function addUvalue() {
    ...
    try {
      ...
      setNewUvalueElement("");
      setAddingUvalue(false);
    } catch (e) { console.error(e); }
  }

  async function deleteUvalue(uid) {
    try {
      await api(`/api/projects/${projectId}/uvalues/${uid}`, { method: "DELETE" });
      setData(d => ({ ...d, uvalues: d.uvalues.filter(u => u.id !== uid) }));
    } catch (e) { console.error(e); }
  }
  ...
  async function addNote() {
    ...
    try {
      const { note } = await api(`/api/projects/${projectId}/notes`, { method: "POST", body: newNote });
      setData(d => ({ ...d, notes: [...d.notes, note] }));
      setNewNote({ label: "", value: "" });
      setAddingNote(false);
    } catch (e) { console.error(e); }
  }
  ...
  async function updateNote(nid, field, value) {
    ...
    try {
      await api(`/api/projects/${projectId}/notes/${nid}`, { method: "PATCH", body: updated });
      setData(d => ({ ...d, notes: d.notes.map(n => n.id === nid ? updated : n) }));
    } catch (e) { console.error(e); }
  }

  async function deleteNote(nid) {
    try {
      await api(`/api/projects/${projectId}/notes/${nid}`, { method: "DELETE" });
      setData(d => ({ ...d, notes: d.notes.filter(n => n.id !== nid) }));
    } catch (e) { console.error(e); }
  }

  // ── Q&A ─────────────────────────────────────────────────────────────────────
  async function askQuestion() {
    if (!qaQuestion.trim() || qaRunning) return;
...
CONSULTANTS:
${consultants.length === 0 ? "None recorded." : consultants.map(c =>
  `${c.discipline || "Unknown discipline"} — ${c.company || ""}${c.contact_name ? ` (${c.contact_name})` : ""}${c.email ? ` · ${c.email}` : ""}${c.phone ? ` · ${c.phone}` : ""}`
).join("\n")}

U-VALUE REQUIREMENTS:
${uvalues.length === 0 ? "None recorded." : uvalues.map(u =>
  `${u.element}: Target ${u.target !== null ? u.target + " W/m²K" : "not set"}, Achieved ${u.achieved !== null ? u.achieved + " W/m²K" : "not set"}${u.notes ? ` — ${u.notes}` : ""}`
).join("\n")}
...
${notes.length === 0 ? "None recorded." : notes.map(n => `${n.label}: ${n.value}`).join("\n")}`;
    ...
    try {
      const { text } = await callClaude(
        ...
        `You are a helpful assistant at an architectural practice. Answer questions about the project using only the information provided. Be concise and practical. If the information needed isn't in the project data, say so clearly.`,
        2000, 1, "gemini-2.5-flash"
      );
      setQaAnswer(text);
      setQaStatus("Answer ready.");
    } catch (e) {
      setQaStatus("Error: " + e.message);
    }
    setQaRunning(false);
  }

  if (loading) return (
    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, color: "#9a9088" }}>
  ...
  const tabs = ["info", "consultants", "u-values", "notes", "ask"];
  ...
  const tabStyle = (t) => ({
    ...
    color: activeTab === t ? ARC_NAVY : "#9a9088",
      ...
      <div style={{ background: "#ffffff", borderBottom: "1px solid #e8e0d5", padding: "20px 32px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 16, marginBottom: 16 }}>
          <button className="btn" onClick={onBack}
            style={{ background: "none", color: "#9a9088", fontSize: 13, padding: "4px 0", border: "none", display: "flex", alignItems: "center", gap: 6, flexShrink: 0, marginTop: 2 }}>
            ← Back
          </button>
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
              ...
              {project.job_number && (
                <span style={{ fontSize: 11, color: "#9a9088", background: ARC_STONE, padding: "2px 8px", fontWeight: 500 }}>#{project.job_number}</span>
              )}
              {project.stage && (
                <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: sColor, background: `${sColor}18`, padding: "3px 8px" }}>
                  {project.stage.split("—")[0].trim()}
                </span>
              )}
            </div>
          ...
          {isAdmin && (
            <button className="btn" onClick={() => { setEditForm({ ...project }); setEditingProject(true); }}
              style={{ background: "none", color: "#9a9088", border: "1px solid #ddd8d0", padding: "6px 14px", fontSize: 11, flexShrink: 0 }}>
              Edit
            </button>
          )}
        </div>
        ...
        {/* Tabs */}
        <div style={{ display: "flex", borderBottom: "1px solid #e8e0d5", marginBottom: -1 }}>
          {tabs.map(t => (
            <button key={t} className="btn" style={tabStyle(t)} onClick={() => setActiveTab(t)}>
              {t === "u-values" ? "U-Values" : t === "ask" ? "Ask" : t.charAt(0).toUpperCase() + t.slice(1)}
            </button>
          ))}
        </div>
      </div>
          ...
          <div style={{ background: "#fff", padding: "32px", width: 560, borderTop: `3px solid ${ARC_TERRACOTTA}`, fontFamily: "Inter, Arial, sans-serif", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <select value={editForm.stage || ""} onChange={e => setEditForm(f => ({ ...f, stage: e.target.value }))}
                style={{ width: "100%", border: "1px solid #ddd8d0", padding: "8px 12px", fontSize: 13, fontFamily: "Inter, Arial, sans-serif", color: editForm.stage ? ARC_NAVY : "#9a9088" }}>
                <option value="">Select stage…</option>
                {RIBA_STAGES.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div style={{ marginBottom: 20 }}>
              ...
              <select value={editForm.status || "active"} onChange={e => setEditForm(f => ({ ...f, status: e.target.value }))}
                style={{ width: "100%", border: "1px solid #ddd8d0", padding: "8px 12px", fontSize: 13, fontFamily: "Inter, Arial, sans-serif" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button className="btn" onClick={saveEditForm} disabled={saving.editForm}
                ...
                {saving.editForm ? <Spinner size={12} /> : "Save"}
              </button>
              <button className="btn" onClick={() => setEditingProject(false)}
                style={{ background: "none", color: "#9a9088", padding: "9px 16px", fontSize: 11, border: "1px solid #ddd8d0" }}>
                Cancel
              </button>
            </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "28px 32px" }}>
        ...
        {activeTab === "info" && (
          <div style={{ maxWidth: 700 }}>
            ...
            <div style={{ fontSize: 11, color: "#b0a8a0", fontStyle: "italic" }}>
            ...
            {addingConsultant && (
              <div style={{ background: "#fff", border: `1px solid ${AD_GREEN}`, padding: "20px 24px", marginBottom: 16 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
                  {[
                    ["discipline", "Discipline", "e.g. Structural Engineer"],
                    ...
                    <div key={field} style={{ marginBottom: 12 }}>
                      <label style={{ fontSize: 10, fontWeight: 600, color: "#9a9088", letterSpacing: "0.08em", textTransform: "uppercase", display: "block", marginBottom: 4 }}>{label}</label>
                      <input value={newConsultant[field]} onChange={e => setNewConsultant(c => ({ ...c, [field]: e.target.value }))}
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                  <button className="btn" onClick={addConsultant} disabled={saving.consultant}
                    ...
                    {saving.consultant ? <Spinner size={12} /> : "Add"}
                  </button>
                  <button className="btn" onClick={() => { setAddingConsultant(false); setNewConsultant({ discipline: "", company: "", contact_name: "", email: "", phone: "" }); }}
                    style={{ background: "none", color: "#9a9088", padding: "7px 14px", fontSize: 11, border: "1px solid #ddd8d0" }}>Cancel</button>
                </div>
              </div>
            )}

            {consultants.length === 0 && !addingConsultant ? (
              <div style={{ background: "#fff", border: "1px solid #e8e0d5", padding: "40px", textAlign: "center" }}>
                ...
                {consultants.map(c => (
                  <div key={c.id} style={{ background: "#fff", border: "1px solid #e8e0d5", padding: "14px 20px" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
                      ...
                      {isAdmin && (
                        <button className="btn" onClick={() => deleteConsultant(c.id)}
          ...
          <div style={{ maxWidth: 700 }}>
            ...
            {addingUvalue && (
              <div style={{ background: "#fff", border: `1px solid ${AD_GREEN}`, padding: "16px 20px", marginBottom: 16, display: "flex", gap: 10, alignItems: "center" }}>
                <input value={newUvalueElement} onChange={e => setNewUvalueElement(e.target.value)}
                  placeholder="Element name e.g. Flat Roof" autoFocus
                ...
                <button className="btn" onClick={addUvalue}
                  style={{ background: AD_GREEN, color: "#fff", padding: "7px 16px", fontSize: 11, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>Add</button>
                  ...
                  {uvalues.map((u, i) => (
                    <tr key={u.id} style={{ background: i % 2 === 0 ? "#faf8f5" : "#fff" }}>
                      ...
                      <td style={{ padding: "9px 14px", borderBottom: "1px solid #e8e0d5" }}>
                        {isAdmin ? (
                          <input type="number" step="0.01" defaultValue={u.target ?? ""} placeholder="—"
                            onBlur={e => updateUvalue(u.id, "target", e.target.value)}
                            style={{ width: 80, border: "1px solid #e8e0d5", padding: "3px 6px", fontSize: 12, fontFamily: "Inter, Arial, sans-serif", color: ARC_NAVY, outline: "none", background: "transparent" }} />
                        ) : (
                      ...
                      <td style={{ padding: "9px 14px", borderBottom: "1px solid #e8e0d5" }}>
                        {isAdmin ? (
                          <input type="number" step="0.01" defaultValue={u.achieved ?? ""} placeholder="—"
                            onBlur={e => updateUvalue(u.id, "achieved", e.target.value)}
                            style={{ width: 80, border: "1px solid #e8e0d5", padding: "3px 6px", fontSize: 12, fontFamily: "Inter, Arial, sans-serif", color: u.achieved !== null && u.target !== null ? (u.achieved <= u.target ? AD_GREEN : ARC_TERRACOTTA) : ARC_NAVY, outline: "none", background: "transparent" }} />
                        ) : (
                          <span style={{ color: u.achieved !== null && u.target !== null ? (u.achieved <= u.target ? AD_GREEN : ARC_TERRACOTTA) : (u.achieved !== null ? ARC_NAVY : "#b0a8a0") }}>
                            {u.achieved !== null ? u.achieved : "—"}
                          </span>
                        )}
                      </td>
                      <td style={{ padding: "9px 14px", borderBottom: "1px solid #e8e0d5" }}>
                        {isAdmin ? (
                          <input defaultValue={u.notes ?? ""} placeholder="Optional notes"
                            onBlur={e => updateUvalue(u.id, "notes", e.target.value)}
                            style={{ width: "100%", border: "1px solid #e8e0d5", padding: "3px 6px", fontSize: 12, fontFamily: "Inter, Arial, sans-serif", color: ARC_NAVY, outline: "none", background: "transparent" }} />
                        ) : (
                      ...
                      <td style={{ padding: "9px 8px", borderBottom: "1px solid #e8e0d5", textAlign: "center" }}>
                        {isAdmin && (
                          <button className="btn" onClick={() => deleteUvalue(u.id)}
                            style={{ background: "none", color: "#c8c0b8", fontSize: 14, padding: "0 2px", border: "none" }}
                            onMouseEnter={e => e.target.style.color = ARC_TERRACOTTA}
                            onMouseLeave={e => e.target.style.color = "#c8c0b8"}>×</button>
                        )}
                      </td>
            ...
            <p style={{ fontSize: 11, color: "#b0a8a0", marginTop: 8, fontStyle: "italic" }}>
              Achieved values shown in green if they meet or beat the target, red if they exceed it.
              {isAdmin && " Click any value to edit it directly."}
            </p>
          </div>
        )}
        ...
        {activeTab === "notes" && (
          <div style={{ maxWidth: 700 }}>
            ...
            {addingNote && (
              <div style={{ background: "#fff", border: `1px solid ${AD_GREEN}`, padding: "16px 20px", marginBottom: 16 }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "0 16px" }}>
                  <div>
                    <label style={{ fontSize: 10, fontWeight: 600, color: "#9a9088", letterSpacing: "0.08em", textTransform: "uppercase", display: "block", marginBottom: 4 }}>Label</label>
                    <input value={newNote.label} onChange={e => setNewNote(n => ({ ...n, label: e.target.value }))}
                      placeholder="e.g. Planning Reference" autoFocus
                  ...
                  <div>
                    ...
                    <input value={newNote.value} onChange={e => setNewNote(n => ({ ...n, value: e.target.value }))}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button className="btn" onClick={addNote}
                    style={{ background: AD_GREEN, color: "#fff", padding: "7px 20px", fontSize: 11, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>Add</button>
            ...
            {notes.length === 0 && !addingNote ? (
              <div style={{ background: "#fff", border: "1px solid #e8e0d5", padding: "40px", textAlign: "center" }}>
                <p style={{ fontSize: 13, color: "#9a9088", fontStyle: "italic" }}>No additional notes yet.</p>
              </div>
            ) : (
              <div style={{ background: "#fff", border: "1px solid #e8e0d5" }}>
                {notes.map((n, i) => (
                  <div key={n.id} style={{ display: "flex", alignItems: "flex-start", gap: 16, padding: "10px 16px", borderBottom: i < notes.length - 1 ? "1px solid #f0ede8" : "none" }}>
                    ...
                    {isAdmin && (
                      <button className="btn" onClick={() => deleteNote(n.id)}
                        style={{ background: "none", color: "#c8c0b8", fontSize: 16, padding: "0 4px", border: "none", flexShrink: 0 }}
                        onMouseEnter={e => e.target.style.color = ARC_TERRACOTTA}
                        onMouseLeave={e => e.target.style.color = "#c8c0b8"}>×</button>
                    )}
        ...
        {activeTab === "ask" && (
          <div style={{ maxWidth: 700 }}>
            {sectionTitle("Ask about this project")}
            <p style={{ fontSize: 12, color: "#9a9088", marginBottom: 20, lineHeight: 1.7 }}>
              Ask any question about this project — consultant contacts, U-value targets, planning references, or anything else recorded here.
            </p>

            <div style={{ display: "flex", gap: 0, marginBottom: 16 }}>
              <textarea value={qaQuestion} onChange={e => setQaQuestion(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); askQuestion(); } }}
                placeholder="e.g. What is the target U-value for the roof? / Who is the structural engineer?"
              ...
              <button className="btn" onClick={askQuestion} disabled={!qaQuestion.trim() || qaRunning}
                style={{ background: qaQuestion.trim() && !qaRunning ? ARC_NAVY : "#f0ede8", color: qaQuestion.trim() && !qaRunning ? "#fff" : "#9a9088", padding: "0 20px", fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", border: `1px solid ${qaQuestion.trim() && !qaRunning ? ARC_NAVY : "#ddd8d0"}`, minWidth: 80 }}>
                {qaRunning ? <Spinner size={12} /> : "Ask"}
              </button>
            </div>

            {qaStatus && (
              <div style={{ fontSize: 12, color: "#505a5f", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
                {qaRunning && <Spinner size={11} />}
                <span>{qaStatus}</span>
              </div>
            )}

            {qaAnswer && (
              <div style={{ background: "#fff", border: "1px solid #e8e0d5", borderTop: `3px solid ${sColor}`, padding: "20px 24px", animation: "fadeIn 0.3s ease" }}>
                <AnswerRenderer text={qaAnswer} />
              </div>
            )}
          </div>
        )}
...
export default function ProjectsSection({ isAdmin }) {
  ...
  async function loadProjects() {
    setLoading(true);
    try {
      const { projects: data } = await api("/api/projects");
      setProjects(data || []);
    } catch (e) { console.error(e); }
    setLoading(false);
  }
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {isAdmin && (
              <button className="btn" onClick={() => setShowNewForm(true)}
                style={{ background: ARC_NAVY, color: "#fff", padding: "8px 20px", fontSize: 11, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase" }}>
                + New Project
              </button>
            )}
          </div>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "80px 40px" }}>
            ...
            <p style={{ fontSize: 12, color: "#9a9088" }}>
              {projects.length === 0 && isAdmin ? "Click + New Project to get started" : "Try a different filter"}
            </p>
          </div>
        ) : (
