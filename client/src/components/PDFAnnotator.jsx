...
const HANDLE_R     = 6; // hit-test radius for handles (px)
...
function getHandles(ann) {
  switch (ann.type) {
    case "leader": return [{ id:"arrow", x:ann.ax, y:ann.ay }, { id:"box", x:ann.tx, y:ann.ty }];
...
function hitTestAnnotation(ann, pt) {
  ...
  switch (ann.type) {
    ...
    case "leader": {
      const fs = ann.fs || (11+(ann.sw-1)*3), pad=5;
      if (pt.x>=ann.tx-pad && pt.x<=ann.tx+160 && pt.y>=ann.ty-fs-pad && pt.y<=ann.ty+pad) return true;
...
function applyHandleDrag(ann, handleId, dx, dy) {
  ...
  switch (ann.type) {
    case "leader":
      ...
      else if (handleId==="box")   { a.tx+=dx; a.ty+=dy; }
...
function drawAnnotation(ctx, ann) {
  ...
  switch (ann.type) {
    ...
    case "leader": {
      ...
      if (!ann.text) break;
      const lfs = ann.fs || (11+(ann.sw-1)*3), pad=5;
      ctx.font = `bold ${lfs}px Inter, Arial, sans-serif`;
      const tw = ctx.measureText(ann.text).width;
      const bx=ann.tx-pad, by=ann.ty-lfs-pad, bw=tw+pad*2, bh=lfs+pad*2;
      ctx.save();
      ...
      ctx.restore();
      ctx.strokeStyle=ann.color||"#e53935"; ctx.fillStyle=ann.color||"#e53935"; ctx.lineWidth=ann.sw||2;
      ctx.fillText(ann.text, ann.tx, ann.ty-pad); break;
    }
    default: break;
  }
}
...
function drawSelectionOverlay(ctx, ann) {
  ...
  switch (ann.type) {
    ...
    case "leader": {
      const lfs=(ann.fs||(11+(ann.sw-1)*3)), pad=5;
      ctx.strokeRect(ann.tx-pad-3, ann.ty-lfs-pad-3, 170, lfs+pad*2+6); break;
    }
    default: break;
  }
  ctx.setLineDash([]);
...
export default function PDFAnnotator({ roundId, taskTitle, roundNumber, onClose, onComplete }) {
  ...
  const dragStateRef     = useRef(null); // { handleId, startPt, origAnn } for select-mode drag
  const PAGE_WIDTH       = 820;

  // Derived: selected annotation on current page
  ...
  // ESC = deselect
  useEffect(() => {
    const onKey = e => { if (e.key === "Escape") { setSelected(null); dragStateRef.current = null; setTextInput(null); pendingLeaderRef.current = null; } };
  ...
  function onMouseDown(e) {
    ...
    if (tool === "select") {
      ...
      // Check handles of currently selected annotation first
      if (selectedAnn) {
      ...
      // Hit-test all annotations (reverse = top-most first)
  ...
  function commitText(text) {
    ...
    if (pendingLeaderRef.current) {
      ...
      const ann = { type:"leader", ax, ay, tx, ty, text:text.trim(), color, sw, fs };
  ...
  async function addComment() {
    ...
    const created = await api(`/api/review-rounds/${roundId}/comments`, { method:"POST", body:{ comment_text:newComment.trim(), page_number:currentPage } }).catch(()=>null);
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
            ...
            {textInput && (
              <input autoFocus
                ...
                style={{ position:"absolute", left:textInput.x, top:textInput.y,
                  ...
                  minWidth:140, fontFamily:"Inter, Arial, sans-serif" }}
              />
            )}
          </div>
        </div>
        ...
        <div style={{ width:300, background:"#1e2a35", borderLeft:"1px solid #2a3a4a", display:"flex", flexDirection:"column", flexShrink:0 }}>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"8px 0" }}>
            {comments.length===0 && <div style={{ padding:"24px 16px", color:"#5a6a7a", fontSize:12, textAlign:"center" }}>No comments yet</div>}
            {comments.map(c => (
